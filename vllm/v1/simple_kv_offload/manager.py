            ...
            for index, source in enumerate(group):
                if source.is_cpu:
                    ...
                    if not target.is_null:
                        gpu_ids.append(target.block_id)
                        cpu_ids.append(source.block.block_id)
        if cpu_ids:
            assert self._gpu_block_pool is not None
            self.cpu_block_pool.touch([self.cpu_block_pool.blocks[i] for i in cpu_ids])
            self._gpu_block_pool.touch(
                [self._gpu_block_pool.blocks[i] for i in gpu_ids]
            )
            assert request.request_id not in self._reqs_to_load
            self._reqs_to_load[request.request_id] = LoadRequestState(
                request, TransferMeta(gpu_ids, cpu_ids)
            )
        hit.release()
